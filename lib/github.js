...
class GitHub {
   ...
   * When creating and `createBody` is given, the comment is first posted with
   * that lighter body and immediately edited to the full one: GitHub only
   * sends notification emails for comment creation, so the email carries the
   * light (image-free) version while the PR page shows the full report.
   */
  async upsertComment(prNumber, marker, body, createBody) {
    let existing = null;
    ...
    const created = await this.request('POST', `/repos/${this.repo}/issues/${prNumber}/comments`, {
      body: createBody || body,
    });
    if (createBody && createBody !== body) {
      return this.request('PATCH', `/repos/${this.repo}/issues/comments/${created.id}`, { body });
    }
    return created;
  }
