...
async function main() {
  ...
  if (mode === 'auto') {
    mode = event.pull_request ? 'diff' : 'promote';
  }
  ...
  if (mode === 'promote') {
    await promote({ gh, remote, branch, baselineRepoDir, snapshotsDir });
  } else {
 ...
 */
async function promote({ gh, remote, branch, baselineRepoDir, snapshotsDir }) {
  const targetBranch = input('base', process.env.GITHUB_REF_NAME || '');
...
async function diff({ gh, repo, remote, branch, baselineRepoDir, snapshotsDir, workDir, event }) {
  ...
  if (!pr) throw new Error('nitpix: diff mode requires a pull_request event (or set mode/base explicitly)');
  const prNumber = pr.number;
  const headSha = pr.head.sha;
  const baseRef = input('base', pr.base.ref);
