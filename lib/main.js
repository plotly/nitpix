...
async function diff({ gh, repo, remote, branch, baselineRepoDir, snapshotsDir, workDir, event }) {
  ...
  // Small thumbnails for the comment (full-size PNGs are only a click away).
  // GitHub loads every inline image in a comment at once — even inside closed
  // <details> — so embedding full screenshots makes big reports very heavy.
  const thumbsDir = path.join(workDir, 'thumbs');
  const thumbJobs = [];
  for (const r of visual) {
    thumbJobs.push({ src: path.join(snapshotsDir, r.name), kind: 'snapshot', name: r.name });
    if (r.status === 'changed') {
      thumbJobs.push({ src: path.join(baselineDir, r.name), kind: 'baseline', name: r.name });
      const diffImg = path.join(diffDir, r.name);
      if (!r.layoutDiff && fs.existsSync(diffImg)) {
        thumbJobs.push({ src: diffImg, kind: 'diff', name: r.name });
      }
    }
  }
  const thumbWidth = parseInt(input('thumbnail-width', '360'), 10);
  const hasThumbs = await makeThumbs(thumbJobs, thumbsDir, thumbWidth);
  ...
  try {
    pushedSha = commitAndPush(
      ...
      (dir) => {
        ...
        if (hasThumbs) {
          for (const { kind, name } of thumbJobs) {
            copyFile(path.join(thumbsDir, kind, name), path.join(dir, pendingDir, 'thumb', kind, name));
          }
        }
  ...
  const urls = rawUrls({ repo, commitSha: pushedSha, baseRef, pendingDir, hasThumbs });
  const reportArgs = {
    results,
    baseRef,
    headSha,
    ...
    approvedNames,
    unapproved,
    maxImages: parseInt(input('max-comment-images', '30'), 10),
    commandPrefix,
    runUrl,
  };
  const body = buildReport(reportArgs);
  // Links-only variant used as the creation body: GitHub emails the comment
  // as created, so notifications stay light; the immediate edit to the full
  // body is what people see on the PR page.
  const emailBody = buildReport({ ...reportArgs, inlineImages: false });
  ...
  if (boolInput('comment', true)) {
    try {
      const comment = await gh.upsertComment(prNumber, MARKER, body, emailBody);
